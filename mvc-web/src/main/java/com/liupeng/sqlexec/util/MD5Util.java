...
public class MD5Util {
    ...
    private static byte[] hexStringToByte(String hex) {
        /*
         * len为什么是hex.length() / 2 ? 首先，hex是一个字符串，里面的内容是像16进制那样的char数组 用2个16进制数字可以表示1个byte，
     ...
     */
    public static boolean validPasswdMd5(String passwd, String dbPasswd)
    ...
    public static boolean validPasswd(String passwd, String dbPasswd)
        throws NoSuchAlgorithmException, UnsupportedEncodingException {
        if (StringUtils.isBlank(passwd) || StringUtils.isBlank(dbPasswd)) {
            return false;
        }
        return dbPasswd.equals(passwd);
    }
