...
public class ClassUtil {
    ...
    private static Set<String> NORMAL_METHODS = Sets.newHashSet("wait", "equals", "toString", "hashCode", "getClass",
        "notify", "notifyAll");
    ...
    private static void findClassesInJar(URL url, Set<Class<?>> classes, String basePackage)
    ...
    public static void findClassesInSrc(URL url, Set<Class<?>> classes, String basePackage)
        ...
        Arrays.stream(dir.listFiles())
            .filter(file -> file.getName().contains("Impl"))
            .forEach(file -> {
                String className = file.getName().substring(0, file.getName().length() - 6);
                try {
                    classes.add(
                        Thread.currentThread().getContextClassLoader().loadClass(basePackage + '.' + className));
    ...
    public static void main(String[] args) {
        // 根据包名路径扫描下面的所有class
        Set<Class<?>> set = ClassUtil.getClasses("com.cjzheng.dubbo.service.impl");
        System.out.println(set.stream().map(Class::getName).collect(Collectors.toList()));
    }
}
